//
// AFTER tritongpu-hoist-tmem-alloc
//
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  tt.func public @attention_forward(%arg0: !ttg.memdesc<256x64xf16, #shared, #smem>, %arg1: !tt.tensordesc<tensor<64x64xf16, #shared>>, %arg2: !tt.tensordesc<tensor<64x64xf16, #shared>>, %arg3: f32, %arg4: i32, %arg5: !tt.ptr<f32>) {
    ...
    // Hoisted QK TMEM alloc
    // use_acc=False and no use outside of loop, so no initial value is needed
    %result, %token = ttng.tmem_alloc : () -> (!ttg.memdesc<256x64xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.async.token)

    // Hoisted PV TMEM alloc
    ...
    // Rotated store of o's initial value:
    //
    // o_final = for (o = dense<0>)
    //   o *= alpha
    //   o_tmem = tmem_alloc o
    //   mma p, v, o_tmem
    //   o = tmem_load o_tmem
    ...
    // use(o_final)
    //
    // After rotating yield(tmem_load o_tmem):
    //
    // o_tmem = tmem_alloc
    // tmem_store dense<0>, o_tmem
    // for ()
    //   o = tmem_load o_tmem
    //   o *= alpha
    //   tmem_store o, o_tmem
    ...
    %1:4 = scf.for %arg6 = %c0_i32 to %arg4 step %c64_i32 iter_args(%arg7 = %cst_1, %arg8 = %cst, %arg9 = %token, %arg10 = %0) -> (tensor<256xf32, #ttg.slice<{dim = 1, parent = #blocked}>>, tensor<256xf32, #ttg.slice<{dim = 1, parent = #blocked}>>, !ttg.async.token, !ttg.async.token)  : i32 {
      // Load K
      %2 = tt.descriptor_load %arg1[%arg6, %c0_i32] : !tt.tensordesc<tensor<64x64xf16, #shared>> -> tensor<64x64xf16, #blocked1>
      ...
      %5 = ttng.tc_gen5_mma %arg0, %4, %result[%arg9], %false, %true : !ttg.memdesc<256x64xf16, #shared, #smem>, !ttg.memdesc<64x64xf16, #shared1, #smem>, !ttg.memdesc<256x64xf32, #tmem, #ttng.tensor_memory, mutable>
      ...
      // Load V
      %21 = tt.descriptor_load %arg2[%arg6, %c0_i32] : !tt.tensordesc<tensor<64x64xf16, #shared>> -> tensor<64x64xf16, #blocked1>
      ...
      %25 = ttng.tc_gen5_mma %result_10, %22, %result_2[%24], %true, %true : !ttg.memdesc<256x64xf16, #tmem1, #ttng.tensor_memory>, !ttg.memdesc<64x64xf16, #shared, #smem>, !ttg.memdesc<256x64xf32, #tmem, #ttng.tensor_memory, mutable>
